			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.UNIDED_WEP_ARM_RING_ART_WAND && (!item.isIdentified() && (item instanceof MeleeWeapon || item instanceof Armor || item instanceof Ring || item instanceof Artifact || item instanceof Wand)) ||
