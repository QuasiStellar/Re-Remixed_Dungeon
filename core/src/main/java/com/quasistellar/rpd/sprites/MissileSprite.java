...
package com.quasistellar.rpd.sprites;
...
public class MissileSprite extends ItemSprite implements Tweener.Listener {
	...
	public void reset( int from, int to, Item item, Callback listener ) {
		reset( DungeonTilemap.tileToWorld( from ), DungeonTilemap.tileToWorld( to ), item, 1f, listener);
	}

	public void reset( int from, int to, Item item, float velocity, Callback listener ) {
		reset( DungeonTilemap.tileToWorld( from ), DungeonTilemap.tileToWorld( to ), item, velocity, listener);
	}

	public void reset( Visual from, Visual to, Item item, Callback listener ) {
		reset(from.center(this), to.center(this), item, 1f, listener );
	}

	public void reset( Visual from, int to, Item item, Callback listener ) {
		reset(from.center(this), DungeonTilemap.tileToWorld( to ), item, 1f, listener );
	}
	
	public void reset( int from, Visual to, Item item, Callback listener ) {
		reset(DungeonTilemap.tileToWorld( from ), to.center(this), item, 1f, listener );
	}

	public void reset( PointF from, PointF to, Item item, Callback listener) {
		reset(from, to, item, 1f, listener);
	}

	public void reset( PointF from, PointF to, Item item, float velocity, Callback listener) {
		revive();
	...
	private void setup( PointF from, PointF to, Item item, float velocity, Callback listener ){
