...
import com.quasistellar.rpd.items.food.FrozenCarpaccio;
import com.quasistellar.rpd.items.food.MysteryMeat;
import com.quasistellar.rpd.items.journal.DocumentPage;
...
public class Heap implements Bundlable {
	...
	public void open( Hero hero ) {
		...
		case TOMB:
			...
			Statistics.tombsPlundered++;
			Badges.validateNecromancerUnlock();
			break;
		case REMAINS:
		case SKELETON:
		...
		if (type != Type.HEAP) {
			...
			for (Item item : items.toArray( new Item[0] )) {
				...
				//unique and upgraded items can endure the blast
						...
						|| (item instanceof MeleeWeapon && ((MeleeWeapon) item).checkSkull() != null)))
					items.remove( item );
