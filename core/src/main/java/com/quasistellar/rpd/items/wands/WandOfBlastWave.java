...
import com.quasistellar.rpd.effects.Effects;
		...
		Actor.addDelayed(new Pushing(ch, ch.pos, newPos, new Callback() {
			public void call() {
				...
				if (ch.pos == trajectory.collisionPos && ch.isAlive()) {
					ch.damage(Random.NormalIntRange((finalDist + 1) / 2, finalDist), WandOfBlastWave.class);
					Paralysis.prolong(ch, Paralysis.class, Random.NormalIntRange((finalDist + 1) / 2, finalDist));
					if (!ch.isAlive() && ch == Dungeon.hero) {
						Dungeon.fail( WandOfBlastWave.class );
						GLog.n( Messages.get(WandOfBlastWave.class, "ondeath") );
					}
				}
