...
import com.quasistellar.rpd.journal.Catalog;
...
public class Badges {
	...
	public static void validateElfUnlock(){
		if (Statistics.potionsCooked >= 3 && !global.contains(Badge.UNLOCK_ELF)){
			displayBadge( Badge.UNLOCK_ELF );
		}
	}

	public static void validateNecromancerUnlock(){
		if (Statistics.tombsPlundered >= 5 && !global.contains(Badge.UNLOCK_NECROMANCER)){
			displayBadge( Badge.UNLOCK_NECROMANCER );
		}
	}

	public static void validateGnollUnlock(){
		if (Statistics.strengthUsed >= 3 && !global.contains(Badge.UNLOCK_GNOLL)){
			displayBadge( Badge.UNLOCK_GNOLL );
		}
	}
	...
	public static void validateBrinkOfDeath() {
		if (!local.contains( Badge.BRINK_OF_DEATH ) && Dungeon.depth >= 10) {
			Badge badge = Badge.BRINK_OF_DEATH;
			local.add( badge );
			displayBadge( badge );
		}
	}

	public static void validatePlagueDoctor() {
		if (!local.contains( Badge.PLAGUE_DOCTOR )) {
			Badge badge = Badge.BRINK_OF_DEATH;
			local.add( badge );
			displayBadge( badge );
		}
